...
var showmodal = function (options) {
    var defaults = {
        ...
        iframePadding: true, //当body的内容为iframe的时候，是否设置padding，默认为true
        /**
        * 初始化DOM结构
        */
        initDom: function () {
        ...
        initCss: function () {
            ...
            if (!this.isText) {
                if (!this.iframePadding) {
                    $("#showModal" + this.modalIndex + " .modal-dialog .modal-body").css({ "padding": "0","overflow":"hidden" });
                }
...
function MoveDialog(ID) {
    ...
    var $head = "";
    if (ID.indexOf("showModal")> -1 ) {
        $head = $dialog.children().eq(0);
    } else {
        $head = $dialog.children().children().eq(0);
    }
    var move = {
        isMove: false,
        left: 0,
